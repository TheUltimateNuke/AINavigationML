...
public class UnityDtCrowdAgent : MonoBehaviour
{
    ...
    public bool SetMoveTarget(RcVec3f pos, bool adjust)
    {
        ...
        var ext = crowdManager.GetQueryExtents() * 100;
        ...
        {
            var retPoly = crowdManager.GetNavMeshQuery().FindNearestPoly(pos, ext, filter, out var nearestRef, out var nearestPt, out _);
            Entrypoint.Logger.Msg($"(agent id {CrowdAgent.idx}) FindNearestPoly Success: {retPoly.Succeeded()} REF {nearestRef}");
            if (CrowdAgent == null)
            {
                foreach (var ag in crowdManager.GetActiveAgents())
                {
    ...
    private void Start()
    {
        if (crowdManager == null || unityAgent == null) return;
        var ext = crowdManager.GetQueryExtents();
        var retPoly = crowdManager.GetNavMeshQuery().FindNearestPoly(transform.position.ToRightHand(), ext, new DtQueryDefaultFilter(), out var nearestRef, out var nearestPt, out _);
        Entrypoint.Logger.Msg("NEAREST REF: " + nearestRef);
        if (retPoly.Failed()) return;
        CrowdAgent = crowdManager.AddAgent(nearestPt, new DtCrowdAgentParams
        {
            radius = 0.1f,
            height = 1.4f,
            maxSpeed = 16,
